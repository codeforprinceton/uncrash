...
Accidentfile <- do.call(rbind, lapply(list.files(pattern = "*Accidents.txt"), fread))
...
colnames(Accidentfile) <- c("Case.Number","County.Name","Municipality.Name", 
                            ...
                            "Reporting.Badge.No"  )
drivers <- do.call(rbind, lapply(list.files(pattern = "*Drivers.txt"), fread))
...
write.csv(FilteredData, file = "FilteredData.csv")
